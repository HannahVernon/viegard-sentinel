...
- **Approval:** Explicitly approved by Hannah ("let's do the insert-only reference tables option").
...
- **Approval:** Explicitly decided by Hannah (web-facing requirement, unique-certificate goal, dedicated-IP topology, AllowedSources with warning).
...
- **Approval:** Explicitly decided by Hannah (periods, keep-corrections-forever, worker mechanism over partitioning).
