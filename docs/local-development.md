...
The `EventKind` target matches the namespaced kind name of any event instead of an HTTP field: `mdaemon/ScreeningBlocked`, `mdaemon/IpBlocked`, `admin/LoginFailed`, `http/request`, and so on.  A `Prefix` match on `mdaemon/` selects the whole source family.  Evidence weights up to 5.0 are allowed; because deterministic confidence is total incident evidence divided by 5, a weight of 3.0 or more lets a single matched event reach the 0.7 review gate and produce a Require-approval decision on its own.
