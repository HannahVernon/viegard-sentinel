// <auto-generated />
using System;
using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Infrastructure;
using Microsoft.EntityFrameworkCore.Migrations;
using Microsoft.EntityFrameworkCore.Storage.ValueConversion;
using Npgsql.EntityFrameworkCore.PostgreSQL.Metadata;
using Viegard.Persistence.Postgres;

#nullable disable

namespace Viegard.Persistence.Postgres.Migrations
{
    [DbContext(typeof(ViegardDbContext))]
    [Migration("20260925172259_AddDohProbeSummary")]
    partial class AddDohProbeSummary
    {
        /// <inheritdoc />
        protected override void BuildTargetModel(ModelBuilder modelBuilder)
        {
#pragma warning disable 612, 618
            modelBuilder
                .HasAnnotation("ProductVersion", "10.0.11")
                .HasAnnotation("Relational:MaxIdentifierLength", 63);

            NpgsqlModelBuilderExtensions.UseIdentityByDefaultColumns(modelBuilder);

            modelBuilder.Entity("Viegard.Persistence.Postgres.Model.ActionProviderRow", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer")
                        .HasColumnName("id");

                    NpgsqlPropertyBuilderExtensions.UseIdentityAlwaysColumn(b.Property<int>("Id"));

                    b.Property<DateTimeOffset>("FirstSeenAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("first_seen_at");

                    b.Property<string>("ProviderKey")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("provider_key");

                    b.HasKey("Id");

                    b.HasIndex("ProviderKey")
                        .IsUnique();

                    b.ToTable("action_providers", (string)null);
                });

            modelBuilder.Entity("Viegard.Persistence.Postgres.Model.ActionRecordRow", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<DateTimeOffset?>("CompletedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("completed_at");

                    b.Property<Guid>("DecisionId")
                        .HasColumnType("uuid")
                        .HasColumnName("decision_id");

                    b.Property<string>("Error")
                        .HasColumnType("text")
                        .HasColumnName("error");

                    b.Property<string>("OperationId")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("operation_id");

                    b.Property<string>("ParametersJson")
                        .HasColumnType("jsonb")
                        .HasColumnName("parameters_json");

                    b.Property<int>("ProviderId")
                        .HasColumnType("integer")
                        .HasColumnName("provider_id");

                    b.Property<DateTimeOffset>("RequestedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("requested_at");

                    b.Property<string>("ResultsJson")
                        .HasColumnType("text")
                        .HasColumnName("results_json");

                    b.Property<string>("RollbackJson")
                        .HasColumnType("jsonb")
                        .HasColumnName("rollback_json");

                    b.Property<int>("Status")
                        .HasColumnType("integer")
                        .HasColumnName("status");

                    b.HasKey("Id");

                    b.HasIndex("DecisionId");

                    b.HasIndex("ProviderId");

                    b.HasIndex("RequestedAt");

                    b.ToTable("actions", (string)null);
                });

            modelBuilder.Entity("Viegard.Persistence.Postgres.Model.ActiveBanRow", b =>
                {
                    b.Property<Guid>("Id")
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<Guid>("ActionId")
                        .HasColumnType("uuid")
                        .HasColumnName("action_id");

                    b.Property<DateTimeOffset>("CreatedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("created_at");

                    b.Property<Guid>("DecisionId")
                        .HasColumnType("uuid")
                        .HasColumnName("decision_id");

                    b.Property<DateTimeOffset>("ExpiresAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("expires_at");

                    b.Property<string>("Ip")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("ip");

                    b.HasKey("Id");

                    b.HasIndex("Ip")
                        .IsUnique();

                    b.ToTable("active_bans", (string)null);
                });

            modelBuilder.Entity("Viegard.Persistence.Postgres.Model.AdminErrorRow", b =>
                {
                    b.Property<Guid>("Id")
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<string>("ExceptionType")
                        .IsRequired()
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)")
                        .HasColumnName("exception_type");

                    b.Property<string>("Message")
                        .IsRequired()
                        .HasMaxLength(2000)
                        .HasColumnType("character varying(2000)")
                        .HasColumnName("message");

                    b.Property<string>("Method")
                        .IsRequired()
                        .HasMaxLength(16)
                        .HasColumnType("character varying(16)")
                        .HasColumnName("method");

                    b.Property<DateTimeOffset>("OccurredAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("occurred_at");

                    b.Property<string>("Path")
                        .IsRequired()
                        .HasMaxLength(512)
                        .HasColumnType("character varying(512)")
                        .HasColumnName("path");

                    b.Property<string>("RequestId")
                        .IsRequired()
                        .HasMaxLength(128)
                        .HasColumnType("character varying(128)")
                        .HasColumnName("request_id");

                    b.Property<string>("StackTrace")
                        .IsRequired()
                        .HasMaxLength(8000)
                        .HasColumnType("character varying(8000)")
                        .HasColumnName("stack_trace");

                    b.Property<string>("Username")
                        .HasMaxLength(128)
                        .HasColumnType("character varying(128)")
                        .HasColumnName("username");

                    b.HasKey("Id");

                    b.HasIndex("OccurredAt")
                        .IsDescending();

                    b.ToTable("admin_errors", (string)null);
                });

            modelBuilder.Entity("Viegard.Persistence.Postgres.Model.AdminRecoveryCodeRow", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<string>("CodeHash")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("code_hash");

                    b.Property<DateTimeOffset>("CreatedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("created_at");

                    b.Property<DateTimeOffset?>("UsedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("used_at");

                    b.Property<Guid>("UserId")
                        .HasColumnType("uuid")
                        .HasColumnName("user_id");

                    b.HasKey("Id");

                    b.HasIndex("UserId");

                    b.ToTable("admin_recovery_codes", (string)null);
                });

            modelBuilder.Entity("Viegard.Persistence.Postgres.Model.AdminSessionRow", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<DateTimeOffset>("AbsoluteExpiresAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("absolute_expires_at");

                    b.Property<DateTimeOffset>("CreatedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("created_at");

                    b.Property<DateTimeOffset>("IdleExpiresAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("idle_expires_at");

                    b.Property<string>("Ip")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("ip");

                    b.Property<string>("IpBindingMode")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("ip_binding_mode");

                    b.Property<DateTimeOffset>("LastSeenAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("last_seen_at");

                    b.Property<DateTimeOffset?>("RevokedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("revoked_at");

                    b.Property<DateTimeOffset?>("StepUpAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("step_up_at");

                    b.Property<string>("UserAgent")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("user_agent");

                    b.Property<Guid>("UserId")
                        .HasColumnType("uuid")
                        .HasColumnName("user_id");

                    b.HasKey("Id");

                    b.HasIndex("AbsoluteExpiresAt");

                    b.HasIndex("IdleExpiresAt");

                    b.HasIndex("RevokedAt");

                    b.HasIndex("UserId");

                    b.ToTable("admin_sessions", (string)null);
                });

            modelBuilder.Entity("Viegard.Persistence.Postgres.Model.AdminTotpSecretRow", b =>
                {
                    b.Property<Guid>("UserId")
                        .HasColumnType("uuid")
                        .HasColumnName("user_id");

                    b.Property<DateTimeOffset>("EnrolledAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("enrolled_at");

                    b.Property<long?>("LastAcceptedStep")
                        .HasColumnType("bigint")
                        .HasColumnName("last_accepted_step");

                    b.Property<string>("SecretBase32")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("secret_base32");

                    b.HasKey("UserId");

                    b.ToTable("admin_totp_secrets", (string)null);
                });

            modelBuilder.Entity("Viegard.Persistence.Postgres.Model.AdminUserPreferencesRow", b =>
                {
                    b.Property<Guid>("UserId")
                        .HasColumnType("uuid")
                        .HasColumnName("user_id");

                    b.Property<int>("PageSize")
                        .HasColumnType("integer")
                        .HasColumnName("page_size");

                    b.Property<int>("StatusRefreshSeconds")
                        .HasColumnType("integer")
                        .HasColumnName("status_refresh_seconds");

                    b.Property<string>("TimeZoneId")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("time_zone_id");

                    b.Property<DateTimeOffset>("UpdatedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("updated_at");

                    b.HasKey("UserId");

                    b.ToTable("admin_user_preferences", (string)null);
                });

            modelBuilder.Entity("Viegard.Persistence.Postgres.Model.AdminUserRow", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<DateTimeOffset>("CreatedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("created_at");

                    b.Property<int>("FailedLoginCount")
                        .HasColumnType("integer")
                        .HasColumnName("failed_login_count");

                    b.Property<DateTimeOffset?>("LockedUntil")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("locked_until");

                    b.Property<bool>("MustChangePassword")
                        .HasColumnType("boolean")
                        .HasColumnName("must_change_password");

                    b.Property<DateTimeOffset>("PasswordChangedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("password_changed_at");

                    b.Property<string>("PasswordHash")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("password_hash");

                    b.Property<bool>("TotpEnrolled")
                        .HasColumnType("boolean")
                        .HasColumnName("totp_enrolled");

                    b.Property<string>("Username")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("username");

                    b.HasKey("Id");

                    b.HasIndex("Username")
                        .IsUnique();

                    b.ToTable("admin_users", (string)null);
                });

            modelBuilder.Entity("Viegard.Persistence.Postgres.Model.AdminWebAuthnCredentialRow", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<Guid>("Aaguid")
                        .HasColumnType("uuid")
                        .HasColumnName("aaguid");

                    b.Property<DateTimeOffset>("CreatedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("created_at");

                    b.Property<byte[]>("CredentialId")
                        .IsRequired()
                        .HasColumnType("bytea")
                        .HasColumnName("credential_id");

                    b.Property<DateTimeOffset?>("LastUsedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("last_used_at");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("name");

                    b.Property<byte[]>("PublicKey")
                        .IsRequired()
                        .HasColumnType("bytea")
                        .HasColumnName("public_key");

                    b.Property<long>("SignCount")
                        .HasColumnType("bigint")
                        .HasColumnName("sign_count");

                    b.Property<string>("Transports")
                        .HasColumnType("text")
                        .HasColumnName("transports");

                    b.Property<Guid>("UserId")
                        .HasColumnType("uuid")
                        .HasColumnName("user_id");

                    b.HasKey("Id");

                    b.HasIndex("CredentialId")
                        .IsUnique();

                    b.HasIndex("UserId");

                    b.ToTable("admin_webauthn_credentials", (string)null);
                });

            modelBuilder.Entity("Viegard.Persistence.Postgres.Model.AppPasswordRow", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<DateTimeOffset>("CreatedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("created_at");

                    b.Property<DateTimeOffset?>("ExpiresAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("expires_at");

                    b.Property<DateTimeOffset?>("LastUsedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("last_used_at");

                    b.Property<string>("LookupKey")
                        .IsRequired()
                        .HasMaxLength(16)
                        .HasColumnType("character varying(16)")
                        .HasColumnName("lookup_key");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasMaxLength(128)
                        .HasColumnType("character varying(128)")
                        .HasColumnName("name");

                    b.Property<DateTimeOffset?>("RevokedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("revoked_at");

                    b.Property<string>("SecretHash")
                        .IsRequired()
                        .HasMaxLength(64)
                        .HasColumnType("character varying(64)")
                        .HasColumnName("secret_hash");

                    b.Property<Guid>("UserId")
                        .HasColumnType("uuid")
                        .HasColumnName("user_id");

                    b.HasKey("Id");

                    b.HasIndex("LookupKey")
                        .IsUnique();

                    b.HasIndex("UserId");

                    b.ToTable("app_passwords", (string)null);
                });

            modelBuilder.Entity("Viegard.Persistence.Postgres.Model.AuditRecordRow", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<Guid?>("ActionId")
                        .HasColumnType("uuid")
                        .HasColumnName("action_id");

                    b.Property<Guid?>("ClassificationId")
                        .HasColumnType("uuid")
                        .HasColumnName("classification_id");

                    b.Property<Guid?>("DecisionId")
                        .HasColumnType("uuid")
                        .HasColumnName("decision_id");

                    b.Property<string>("DetailJson")
                        .HasColumnType("jsonb")
                        .HasColumnName("detail_json");

                    b.Property<Guid?>("EventId")
                        .HasColumnType("uuid")
                        .HasColumnName("event_id");

                    b.Property<Guid?>("IncidentId")
                        .HasColumnType("uuid")
                        .HasColumnName("incident_id");

                    b.Property<int?>("SourceId")
                        .HasColumnType("integer")
                        .HasColumnName("source_id");

                    b.Property<int>("Stage")
                        .HasColumnType("integer")
                        .HasColumnName("stage");

                    b.Property<string>("Summary")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("summary");

                    b.Property<DateTimeOffset>("Timestamp")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("timestamp");

                    b.HasKey("Id");

                    b.HasIndex("SourceId");

                    b.HasIndex("Timestamp");

                    b.HasIndex("Stage", "Id")
                        .IsDescending(false, true);

                    b.ToTable("audit_records", (string)null);
                });

            modelBuilder.Entity("Viegard.Persistence.Postgres.Model.BurstCooldownRow", b =>
                {
                    b.Property<string>("SignalId")
                        .HasMaxLength(64)
                        .HasColumnType("character varying(64)")
                        .HasColumnName("signal_id");

                    b.Property<string>("SourceKey")
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)")
                        .HasColumnName("source_key");

                    b.Property<DateTimeOffset>("LastFiredAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("last_fired_at");

                    b.HasKey("SignalId", "SourceKey");

                    b.ToTable("burst_cooldowns", (string)null);
                });

            modelBuilder.Entity("Viegard.Persistence.Postgres.Model.BurstDetectionSettingsRow", b =>
                {
                    b.Property<int>("Id")
                        .HasColumnType("integer")
                        .HasColumnName("id");

                    b.Property<bool>("AuthFailureActionEligible")
                        .HasColumnType("boolean")
                        .HasColumnName("auth_failure_action_eligible");

                    b.Property<int>("AuthFailureCooldownSeconds")
                        .HasColumnType("integer")
                        .HasColumnName("auth_failure_cooldown_seconds");

                    b.Property<bool>("AuthFailureEnabled")
                        .HasColumnType("boolean")
                        .HasColumnName("auth_failure_enabled");

                    b.Property<int>("AuthFailureThreshold")
                        .HasColumnType("integer")
                        .HasColumnName("auth_failure_threshold");

                    b.Property<int>("AuthFailureWindowSeconds")
                        .HasColumnType("integer")
                        .HasColumnName("auth_failure_window_seconds");

                    b.Property<bool>("GlobalEnabled")
                        .HasColumnType("boolean")
                        .HasColumnName("global_enabled");

                    b.Property<int>("RowVersion")
                        .HasColumnType("integer")
                        .HasColumnName("row_version");

                    b.Property<DateTimeOffset>("UpdatedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("updated_at");

                    b.Property<string>("UpdatedBy")
                        .IsRequired()
                        .HasMaxLength(128)
                        .HasColumnType("character varying(128)")
                        .HasColumnName("updated_by");

                    b.HasKey("Id");

                    b.ToTable("burst_detection_settings", null, t =>
                        {
                            t.HasCheckConstraint("CK_burst_detection_settings_fixed_id", "id = 1");
                        });
                });

            modelBuilder.Entity("Viegard.Persistence.Postgres.Model.BurstWindowRow", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint")
                        .HasColumnName("id");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<long>("Id"));

                    b.Property<Guid>("EventId")
                        .HasColumnType("uuid")
                        .HasColumnName("event_id");

                    b.Property<DateTimeOffset>("OccurredAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("occurred_at");

                    b.Property<string>("SignalId")
                        .IsRequired()
                        .HasMaxLength(64)
                        .HasColumnType("character varying(64)")
                        .HasColumnName("signal_id");

                    b.Property<string>("SourceKey")
                        .IsRequired()
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)")
                        .HasColumnName("source_key");

                    b.HasKey("Id");

                    b.HasIndex("SignalId", "SourceKey", "OccurredAt");

                    b.ToTable("burst_windows", (string)null);
                });

            modelBuilder.Entity("Viegard.Persistence.Postgres.Model.ClassificationRow", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<string>("Category")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("category");

                    b.Property<int>("ClassifierId")
                        .HasColumnType("integer")
                        .HasColumnName("classifier_id");

                    b.Property<double>("Confidence")
                        .HasColumnType("double precision")
                        .HasColumnName("confidence");

                    b.Property<DateTimeOffset>("CreatedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("created_at");

                    b.Property<string>("ModelJson")
                        .HasColumnType("jsonb")
                        .HasColumnName("model_json");

                    b.Property<string>("ReasonsJson")
                        .IsRequired()
                        .HasColumnType("jsonb")
                        .HasColumnName("reasons_json");

                    b.Property<string>("RecommendedAction")
                        .HasColumnType("text")
                        .HasColumnName("recommended_action");

                    b.Property<int>("Severity")
                        .HasColumnType("integer")
                        .HasColumnName("severity");

                    b.Property<Guid>("SubjectId")
                        .HasColumnType("uuid")
                        .HasColumnName("subject_id");

                    b.Property<int>("SubjectKind")
                        .HasColumnType("integer")
                        .HasColumnName("subject_kind");

                    b.Property<double?>("Uncertainty")
                        .HasColumnType("double precision")
                        .HasColumnName("uncertainty");

                    b.HasKey("Id");

                    b.HasIndex("ClassifierId");

                    b.HasIndex("CreatedAt");

                    b.HasIndex("SubjectId");

                    b.ToTable("classifications", (string)null);
                });

            modelBuilder.Entity("Viegard.Persistence.Postgres.Model.ClassifierRow", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer")
                        .HasColumnName("id");

                    NpgsqlPropertyBuilderExtensions.UseIdentityAlwaysColumn(b.Property<int>("Id"));

                    b.Property<string>("ClassifierKey")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("classifier_key");

                    b.Property<DateTimeOffset>("FirstSeenAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("first_seen_at");

                    b.HasKey("Id");

                    b.HasIndex("ClassifierKey")
                        .IsUnique();

                    b.ToTable("classifiers", (string)null);
                });

            modelBuilder.Entity("Viegard.Persistence.Postgres.Model.ClassifierSettingsRow", b =>
                {
                    b.Property<int>("Id")
                        .HasColumnType("integer")
                        .HasColumnName("id");

                    b.Property<double>("BlockRecommendationScore")
                        .HasColumnType("double precision")
                        .HasColumnName("block_recommendation_score");

                    b.Property<double>("RepeatConfidenceBonusCap")
                        .HasColumnType("double precision")
                        .HasColumnName("repeat_confidence_bonus_cap");

                    b.Property<double>("RepeatConfidenceCoefficient")
                        .HasColumnType("double precision")
                        .HasColumnName("repeat_confidence_coefficient");

                    b.Property<int>("RepeatConfidenceMinEvents")
                        .HasColumnType("integer")
                        .HasColumnName("repeat_confidence_min_events");

                    b.Property<int>("RowVersion")
                        .HasColumnType("integer")
                        .HasColumnName("row_version");

                    b.Property<double>("ScoreForFullConfidence")
                        .HasColumnType("double precision")
                        .HasColumnName("score_for_full_confidence");

                    b.Property<double>("SeverityPerScorePoint")
                        .HasColumnType("double precision")
                        .HasColumnName("severity_per_score_point");

                    b.Property<DateTimeOffset>("UpdatedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("updated_at");

                    b.Property<string>("UpdatedBy")
                        .IsRequired()
                        .HasMaxLength(128)
                        .HasColumnType("character varying(128)")
                        .HasColumnName("updated_by");

                    b.HasKey("Id");

                    b.ToTable("classifier_settings", null, t =>
                        {
                            t.HasCheckConstraint("CK_classifier_settings_fixed_id", "id = 1");
                        });
                });

            modelBuilder.Entity("Viegard.Persistence.Postgres.Model.CorrectionRow", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<Guid>("ClassificationId")
                        .HasColumnType("uuid")
                        .HasColumnName("classification_id");

                    b.Property<string>("CorrectedBy")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("corrected_by");

                    b.Property<string>("CorrectedCategory")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("corrected_category");

                    b.Property<DateTimeOffset>("CreatedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("created_at");

                    b.Property<string>("Note")
                        .HasColumnType("text")
                        .HasColumnName("note");

                    b.HasKey("Id");

                    b.HasIndex("ClassificationId");

                    b.ToTable("corrections", (string)null);
                });

            modelBuilder.Entity("Viegard.Persistence.Postgres.Model.CustomSignatureRow", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<string>("AdditionalPatternsJson")
                        .HasColumnType("text")
                        .HasColumnName("additional_patterns_json");

                    b.Property<string>("Category")
                        .IsRequired()
                        .HasMaxLength(128)
                        .HasColumnType("character varying(128)")
                        .HasColumnName("category");

                    b.Property<DateTimeOffset>("CreatedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("created_at");

                    b.Property<bool>("Enabled")
                        .HasColumnType("boolean")
                        .HasColumnName("enabled");

                    b.Property<double>("EvidenceWeight")
                        .HasColumnType("double precision")
                        .HasColumnName("evidence_weight");

                    b.Property<int>("MatchType")
                        .HasColumnType("integer")
                        .HasColumnName("match_type");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasMaxLength(128)
                        .HasColumnType("character varying(128)")
                        .HasColumnName("name");

                    b.Property<string>("Pattern")
                        .IsRequired()
                        .HasMaxLength(512)
                        .HasColumnType("character varying(512)")
                        .HasColumnName("pattern");

                    b.Property<int>("Severity")
                        .HasColumnType("integer")
                        .HasColumnName("severity");

                    b.Property<int>("Target")
                        .HasColumnType("integer")
                        .HasColumnName("target");

                    b.Property<DateTimeOffset>("UpdatedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("updated_at");

                    b.Property<string>("UpdatedBy")
                        .IsRequired()
                        .HasMaxLength(128)
                        .HasColumnType("character varying(128)")
                        .HasColumnName("updated_by");

                    b.Property<int>("Version")
                        .HasColumnType("integer")
                        .HasColumnName("version");

                    b.HasKey("Id");

                    b.HasIndex("Name")
                        .IsUnique();

                    b.ToTable("custom_signatures", (string)null);
                });

            modelBuilder.Entity("Viegard.Persistence.Postgres.Model.DecisionRow", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<string>("AuthorizedTargetIp")
                        .HasColumnType("text")
                        .HasColumnName("authorized_target_ip");

                    b.Property<Guid>("ClassificationId")
                        .HasColumnType("uuid")
                        .HasColumnName("classification_id");

                    b.Property<DateTimeOffset>("CreatedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("created_at");

                    b.Property<string>("GuardrailsJson")
                        .IsRequired()
                        .HasColumnType("jsonb")
                        .HasColumnName("guardrails_json");

                    b.Property<int>("Outcome")
                        .HasColumnType("integer")
                        .HasColumnName("outcome");

                    b.Property<int>("PolicyId")
                        .HasColumnType("integer")
                        .HasColumnName("policy_id");

                    b.Property<string>("Rationale")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("rationale");

                    b.Property<TimeSpan?>("RecommendedActionDuration")
                        .HasColumnType("interval")
                        .HasColumnName("recommended_action_duration");

                    b.Property<int?>("ReviewOutcome")
                        .HasColumnType("integer")
                        .HasColumnName("review_outcome");

                    b.Property<DateTimeOffset?>("ReviewedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("reviewed_at");

                    b.Property<string>("ReviewedBy")
                        .HasColumnType("text")
                        .HasColumnName("reviewed_by");

                    b.Property<DateTimeOffset?>("SupersededAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("superseded_at");

                    b.Property<Guid?>("SupersededByDecisionId")
                        .HasColumnType("uuid")
                        .HasColumnName("superseded_by_decision_id");

                    b.HasKey("Id");

                    b.HasIndex("ClassificationId");

                    b.HasIndex("CreatedAt");

                    b.HasIndex("PolicyId");

                    b.HasIndex("Outcome", "Id")
                        .IsDescending(false, true);

                    b.ToTable("decisions", (string)null);
                });

            modelBuilder.Entity("Viegard.Persistence.Postgres.Model.DohBlocklistSettingsRow", b =>
                {
                    b.Property<int>("Id")
                        .HasColumnType("integer")
                        .HasColumnName("id");

                    b.Property<string>("AddressListName")
                        .IsRequired()
                        .HasMaxLength(128)
                        .HasColumnType("character varying(128)")
                        .HasColumnName("address_list_name");

                    b.Property<bool>("ApplyToRouters")
                        .HasColumnType("boolean")
                        .HasColumnName("apply_to_routers");

                    b.Property<bool>("Enabled")
                        .HasColumnType("boolean")
                        .HasColumnName("enabled");

                    b.Property<int>("FetchIntervalSeconds")
                        .HasColumnType("integer")
                        .HasColumnName("fetch_interval_seconds");

                    b.Property<string>("PrimaryFeedUrl")
                        .IsRequired()
                        .HasMaxLength(2048)
                        .HasColumnType("character varying(2048)")
                        .HasColumnName("primary_feed_url");

                    b.Property<string>("ProbeCanaryFqdn")
                        .IsRequired()
                        .HasMaxLength(253)
                        .HasColumnType("character varying(253)")
                        .HasColumnName("probe_canary_fqdn");

                    b.Property<int>("ProbeConcurrency")
                        .HasColumnType("integer")
                        .HasColumnName("probe_concurrency");

                    b.Property<bool>("ProbeEnabled")
                        .HasColumnType("boolean")
                        .HasColumnName("probe_enabled");

                    b.Property<string>("ProbeEndpointPath")
                        .IsRequired()
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)")
                        .HasColumnName("probe_endpoint_path");

                    b.Property<string>("ProbeExpectedToken")
                        .IsRequired()
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)")
                        .HasColumnName("probe_expected_token");

                    b.Property<int>("ProbeIntervalSeconds")
                        .HasColumnType("integer")
                        .HasColumnName("probe_interval_seconds");

                    b.Property<int>("ProbeTimeoutSeconds")
                        .HasColumnType("integer")
                        .HasColumnName("probe_timeout_seconds");

                    b.Property<int>("RowVersion")
                        .HasColumnType("integer")
                        .HasColumnName("row_version");

                    b.Property<string>("SecondaryFeedUrl")
                        .IsRequired()
                        .HasMaxLength(2048)
                        .HasColumnType("character varying(2048)")
                        .HasColumnName("secondary_feed_url");

                    b.Property<DateTimeOffset>("UpdatedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("updated_at");

                    b.Property<string>("UpdatedBy")
                        .IsRequired()
                        .HasMaxLength(128)
                        .HasColumnType("character varying(128)")
                        .HasColumnName("updated_by");

                    b.HasKey("Id");

                    b.ToTable("doh_blocklist_settings", null, t =>
                        {
                            t.HasCheckConstraint("CK_doh_blocklist_settings_fixed_id", "id = 1");
                        });
                });

            modelBuilder.Entity("Viegard.Persistence.Postgres.Model.DohDesiredAddressRow", b =>
                {
                    b.Property<string>("Address")
                        .HasColumnType("text")
                        .HasColumnName("address");

                    b.Property<DateTimeOffset>("FirstSeenAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("first_seen_at");

                    b.Property<DateTimeOffset>("LastSeenAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("last_seen_at");

                    b.HasKey("Address");

                    b.ToTable("doh_desired_addresses", (string)null);
                });

            modelBuilder.Entity("Viegard.Persistence.Postgres.Model.DohProbeResultRow", b =>
                {
                    b.Property<string>("Address")
                        .HasColumnType("text")
                        .HasColumnName("address");

                    b.Property<int>("ConsecutiveFailures")
                        .HasColumnType("integer")
                        .HasColumnName("consecutive_failures");

                    b.Property<DateTimeOffset>("FirstSeenAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("first_seen_at");

                    b.Property<int?>("HttpStatus")
                        .HasColumnType("integer")
                        .HasColumnName("http_status");

                    b.Property<DateTimeOffset?>("LastConfirmedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("last_confirmed_at");

                    b.Property<DateTimeOffset>("LastProbedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("last_probed_at");

                    b.Property<int>("Status")
                        .HasColumnType("integer")
                        .HasColumnName("status");

                    b.Property<bool>("TokenMatched")
                        .HasColumnType("boolean")
                        .HasColumnName("token_matched");

                    b.HasKey("Address");

                    b.ToTable("doh_probe_results", (string)null);
                });

            modelBuilder.Entity("Viegard.Persistence.Postgres.Model.DohProbeSummaryRow", b =>
                {
                    b.Property<int>("Id")
                        .HasColumnType("integer")
                        .HasColumnName("id");

                    b.Property<string>("DetailJson")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("detail_json");

                    b.Property<DateTimeOffset>("GeneratedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("generated_at");

                    b.HasKey("Id");

                    b.ToTable("doh_probe_summary", null, t =>
                        {
                            t.HasCheckConstraint("CK_doh_probe_summary_fixed_id", "id = 1");
                        });
                });

            modelBuilder.Entity("Viegard.Persistence.Postgres.Model.DohReconciliationProposalRow", b =>
                {
                    b.Property<int>("Id")
                        .HasColumnType("integer")
                        .HasColumnName("id");

                    b.Property<string>("DetailJson")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("detail_json");

                    b.Property<DateTimeOffset>("GeneratedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("generated_at");

                    b.HasKey("Id");

                    b.ToTable("doh_reconciliation_proposal", null, t =>
                        {
                            t.HasCheckConstraint("CK_doh_reconciliation_proposal_fixed_id", "id = 1");
                        });
                });

            modelBuilder.Entity("Viegard.Persistence.Postgres.Model.HostUpgradeCommandRow", b =>
                {
                    b.Property<Guid>("Id")
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<string>("Detail")
                        .HasColumnType("text")
                        .HasColumnName("detail");

                    b.Property<DateTimeOffset?>("FinishedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("finished_at");

                    b.Property<DateTimeOffset>("RequestedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("requested_at");

                    b.Property<string>("RequestedBy")
                        .IsRequired()
                        .HasMaxLength(128)
                        .HasColumnType("character varying(128)")
                        .HasColumnName("requested_by");

                    b.Property<DateTimeOffset?>("StartedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("started_at");

                    b.Property<int>("Status")
                        .HasColumnType("integer")
                        .HasColumnName("status");

                    b.Property<string>("Target")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("target");

                    b.HasKey("Id");

                    b.HasIndex("Target")
                        .IsUnique()
                        .HasFilter("status IN (0, 1)");

                    b.HasIndex("Target", "FinishedAt");

                    b.HasIndex("Target", "Status", "RequestedAt");

                    b.ToTable("host_upgrade_commands", (string)null);
                });

            modelBuilder.Entity("Viegard.Persistence.Postgres.Model.IncidentCoalescingSettingsRow", b =>
                {
                    b.Property<int>("Id")
                        .HasColumnType("integer")
                        .HasColumnName("id");

                    b.Property<bool>("Enabled")
                        .HasColumnType("boolean")
                        .HasColumnName("enabled");

                    b.Property<int>("MaxCoalesceWindowSeconds")
                        .HasColumnType("integer")
                        .HasColumnName("max_coalesce_window_seconds");

                    b.Property<int>("RowVersion")
                        .HasColumnType("integer")
                        .HasColumnName("row_version");

                    b.Property<int>("SettleWindowSeconds")
                        .HasColumnType("integer")
                        .HasColumnName("settle_window_seconds");

                    b.Property<DateTimeOffset>("UpdatedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("updated_at");

                    b.Property<string>("UpdatedBy")
                        .IsRequired()
                        .HasMaxLength(128)
                        .HasColumnType("character varying(128)")
                        .HasColumnName("updated_by");

                    b.HasKey("Id");

                    b.ToTable("incident_coalescing_settings", null, t =>
                        {
                            t.HasCheckConstraint("CK_incident_coalescing_settings_fixed_id", "id = 1");
                        });
                });

            modelBuilder.Entity("Viegard.Persistence.Postgres.Model.IncidentRow", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<DateTimeOffset?>("CoalesceUntil")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("coalesce_until");

                    b.Property<string>("CorrelationKey")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("correlation_key");

                    b.Property<int>("DecidedEventCount")
                        .HasColumnType("integer")
                        .HasColumnName("decided_event_count");

                    b.Property<string>("EventIdsJson")
                        .IsRequired()
                        .HasColumnType("jsonb")
                        .HasColumnName("event_ids_json");

                    b.Property<string>("EvidenceJson")
                        .IsRequired()
                        .HasColumnType("jsonb")
                        .HasColumnName("evidence_json");

                    b.Property<int>("State")
                        .HasColumnType("integer")
                        .HasColumnName("state");

                    b.Property<DateTimeOffset>("WindowEnd")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("window_end");

                    b.Property<DateTimeOffset>("WindowStart")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("window_start");

                    b.HasKey("Id");

                    b.HasIndex("CorrelationKey", "State");

                    b.HasIndex("State", "Id")
                        .IsDescending(false, true);

                    b.HasIndex("WindowStart", "State");

                    b.ToTable("incidents", (string)null);
                });

            modelBuilder.Entity("Viegard.Persistence.Postgres.Model.IngestionFilterRow", b =>
                {
                    b.Property<string>("SourceType")
                        .HasMaxLength(64)
                        .HasColumnType("character varying(64)")
                        .HasColumnName("source_type");

                    b.Property<string>("EventKind")
                        .HasMaxLength(128)
                        .HasColumnType("character varying(128)")
                        .HasColumnName("event_kind");

                    b.Property<bool>("Suppressed")
                        .HasColumnType("boolean")
                        .HasColumnName("suppressed");

                    b.Property<DateTimeOffset>("UpdatedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("updated_at");

                    b.Property<string>("UpdatedBy")
                        .IsRequired()
                        .HasMaxLength(128)
                        .HasColumnType("character varying(128)")
                        .HasColumnName("updated_by");

                    b.HasKey("SourceType", "EventKind");

                    b.ToTable("ingestion_filters", (string)null);
                });

            modelBuilder.Entity("Viegard.Persistence.Postgres.Model.InstanceRegistrationRow", b =>
                {
                    b.Property<string>("InstanceId")
                        .HasColumnType("text")
                        .HasColumnName("instance_id");

                    b.Property<string>("CommitSha")
                        .HasColumnType("text")
                        .HasColumnName("commit_sha");

                    b.Property<string>("HostName")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("host_name");

                    b.Property<DateTimeOffset>("ReportedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("reported_at");

                    b.Property<string>("Roles")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("roles");

                    b.Property<DateTimeOffset>("StartedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("started_at");

                    b.Property<string>("UpgradeTarget")
                        .HasMaxLength(64)
                        .HasColumnType("character varying(64)")
                        .HasColumnName("upgrade_target");

                    b.Property<string>("Version")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("version");

                    b.HasKey("InstanceId");

                    b.ToTable("instance_registry", (string)null);
                });

            modelBuilder.Entity("Viegard.Persistence.Postgres.Model.JetPackDesiredAddressRow", b =>
                {
                    b.Property<string>("Address")
                        .HasColumnType("text")
                        .HasColumnName("address");

                    b.Property<DateTimeOffset>("FirstSeenAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("first_seen_at");

                    b.Property<DateTimeOffset>("LastSeenAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("last_seen_at");

                    b.HasKey("Address");

                    b.ToTable("jetpack_desired_addresses", (string)null);
                });

            modelBuilder.Entity("Viegard.Persistence.Postgres.Model.JetPackFeedSettingsRow", b =>
                {
                    b.Property<int>("Id")
                        .HasColumnType("integer")
                        .HasColumnName("id");

                    b.Property<string>("AddressListName")
                        .IsRequired()
                        .HasMaxLength(128)
                        .HasColumnType("character varying(128)")
                        .HasColumnName("address_list_name");

                    b.Property<bool>("Enabled")
                        .HasColumnType("boolean")
                        .HasColumnName("enabled");

                    b.Property<string>("FeedUrl")
                        .IsRequired()
                        .HasMaxLength(2048)
                        .HasColumnType("character varying(2048)")
                        .HasColumnName("feed_url");

                    b.Property<TimeSpan>("FetchInterval")
                        .HasColumnType("interval")
                        .HasColumnName("fetch_interval");

                    b.Property<DateTimeOffset?>("SeededAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("seeded_at");

                    b.Property<DateTimeOffset>("UpdatedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("updated_at");

                    b.Property<string>("UpdatedBy")
                        .IsRequired()
                        .HasMaxLength(128)
                        .HasColumnType("character varying(128)")
                        .HasColumnName("updated_by");

                    b.Property<int>("Version")
                        .HasColumnType("integer")
                        .HasColumnName("version");

                    b.HasKey("Id");

                    b.ToTable("jetpack_feed_settings", null, t =>
                        {
                            t.HasCheckConstraint("CK_jetpack_feed_settings_fixed_id", "id = 1");
                        });
                });

            modelBuilder.Entity("Viegard.Persistence.Postgres.Model.LocalModelAdvisorCategoryBandRow", b =>
                {
                    b.Property<string>("Category")
                        .HasMaxLength(64)
                        .HasColumnType("character varying(64)")
                        .HasColumnName("category");

                    b.Property<bool?>("DeEscalationEnabled")
                        .HasColumnType("boolean")
                        .HasColumnName("de_escalation_enabled");

                    b.Property<bool?>("Enabled")
                        .HasColumnType("boolean")
                        .HasColumnName("enabled");

                    b.Property<double?>("InvokeConfidenceMax")
                        .HasColumnType("double precision")
                        .HasColumnName("invoke_confidence_max");

                    b.Property<double?>("InvokeConfidenceMin")
                        .HasColumnType("double precision")
                        .HasColumnName("invoke_confidence_min");

                    b.Property<double?>("MaxConfidenceDelta")
                        .HasColumnType("double precision")
                        .HasColumnName("max_confidence_delta");

                    b.Property<double?>("MaxDownwardConfidenceDelta")
                        .HasColumnType("double precision")
                        .HasColumnName("max_downward_confidence_delta");

                    b.Property<int?>("MaxDownwardSeverityDelta")
                        .HasColumnType("integer")
                        .HasColumnName("max_downward_severity_delta");

                    b.Property<int?>("MaxSeverityDelta")
                        .HasColumnType("integer")
                        .HasColumnName("max_severity_delta");

                    b.Property<DateTimeOffset>("UpdatedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("updated_at");

                    b.Property<string>("UpdatedBy")
                        .IsRequired()
                        .HasMaxLength(128)
                        .HasColumnType("character varying(128)")
                        .HasColumnName("updated_by");

                    b.Property<int>("Version")
                        .HasColumnType("integer")
                        .HasColumnName("version");

                    b.HasKey("Category");

                    b.ToTable("local_model_advisor_category_bands", (string)null);
                });

            modelBuilder.Entity("Viegard.Persistence.Postgres.Model.LocalModelAdvisorConsultRow", b =>
                {
                    b.Property<Guid>("Id")
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<bool>("AdvisorSkippedForInjection")
                        .HasColumnType("boolean")
                        .HasColumnName("advisor_skipped_for_injection");

                    b.Property<double>("BaseConfidence")
                        .HasColumnType("double precision")
                        .HasColumnName("base_confidence");

                    b.Property<int>("BaseSeverity")
                        .HasColumnType("integer")
                        .HasColumnName("base_severity");

                    b.Property<string>("Category")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("category");

                    b.Property<Guid>("ClassificationId")
                        .HasColumnType("uuid")
                        .HasColumnName("classification_id");

                    b.Property<DateTimeOffset>("CreatedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("created_at");

                    b.Property<string>("EnsembleDetailJson")
                        .HasColumnType("jsonb")
                        .HasColumnName("ensemble_detail");

                    b.Property<string>("FailureKind")
                        .HasColumnType("text")
                        .HasColumnName("failure_kind");

                    b.Property<double>("FinalConfidence")
                        .HasColumnType("double precision")
                        .HasColumnName("final_confidence");

                    b.Property<int>("FinalSeverity")
                        .HasColumnType("integer")
                        .HasColumnName("final_severity");

                    b.Property<Guid>("IncidentId")
                        .HasColumnType("uuid")
                        .HasColumnName("incident_id");

                    b.Property<string>("InjectionCategoriesJson")
                        .IsRequired()
                        .ValueGeneratedOnAdd()
                        .HasColumnType("jsonb")
                        .HasDefaultValue("[]")
                        .HasColumnName("injection_categories");

                    b.Property<bool>("InjectionDetected")
                        .HasColumnType("boolean")
                        .HasColumnName("injection_detected");

                    b.Property<int?>("LatencyMs")
                        .HasColumnType("integer")
                        .HasColumnName("latency_ms");

                    b.Property<string>("ModelId")
                        .HasColumnType("text")
                        .HasColumnName("model_id");

                    b.Property<int>("Outcome")
                        .HasColumnType("integer")
                        .HasColumnName("outcome");

                    b.Property<bool>("ServedFromCache")
                        .HasColumnType("boolean")
                        .HasColumnName("served_from_cache");

                    b.HasKey("Id");

                    b.HasIndex("ClassificationId");

                    b.HasIndex("CreatedAt");

                    b.ToTable("local_model_advisor_consults", (string)null);
                });

            modelBuilder.Entity("Viegard.Persistence.Postgres.Model.LocalModelAdvisorInjectionPatternRow", b =>
                {
                    b.Property<Guid>("Id")
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<string>("Category")
                        .IsRequired()
                        .HasMaxLength(64)
                        .HasColumnType("character varying(64)")
                        .HasColumnName("category");

                    b.Property<DateTimeOffset>("CreatedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("created_at");

                    b.Property<string>("CreatedBy")
                        .IsRequired()
                        .HasMaxLength(128)
                        .HasColumnType("character varying(128)")
                        .HasColumnName("created_by");

                    b.Property<string>("Description")
                        .HasMaxLength(512)
                        .HasColumnType("character varying(512)")
                        .HasColumnName("description");

                    b.Property<bool>("Enabled")
                        .HasColumnType("boolean")
                        .HasColumnName("enabled");

                    b.Property<string>("Pattern")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("pattern");

                    b.HasKey("Id");

                    b.HasIndex("CreatedAt");

                    b.ToTable("local_model_advisor_injection_patterns", (string)null);
                });

            modelBuilder.Entity("Viegard.Persistence.Postgres.Model.LocalModelAdvisorPromptTemplateRow", b =>
                {
                    b.Property<Guid>("Id")
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<string>("ApplicationInstructions")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("application_instructions");

                    b.Property<DateTimeOffset>("CreatedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("created_at");

                    b.Property<string>("CreatedBy")
                        .IsRequired()
                        .HasMaxLength(128)
                        .HasColumnType("character varying(128)")
                        .HasColumnName("created_by");

                    b.Property<bool>("IsActive")
                        .HasColumnType("boolean")
                        .HasColumnName("is_active");

                    b.Property<string>("Note")
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)")
                        .HasColumnName("note");

                    b.Property<int>("Revision")
                        .HasColumnType("integer")
                        .HasColumnName("revision");

                    b.Property<string>("SystemInstructions")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("system_instructions");

                    b.Property<string>("TemplateId")
                        .IsRequired()
                        .HasMaxLength(64)
                        .HasColumnType("character varying(64)")
                        .HasColumnName("template_id");

                    b.HasKey("Id");

                    b.HasIndex("TemplateId")
                        .IsUnique()
                        .HasFilter("is_active");

                    b.HasIndex("TemplateId", "Revision")
                        .IsUnique();

                    b.ToTable("local_model_advisor_prompt_templates", (string)null);
                });

            modelBuilder.Entity("Viegard.Persistence.Postgres.Model.LocalModelAdvisorResponseCacheRow", b =>
                {
                    b.Property<Guid>("Id")
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<string>("CacheKey")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("cache_key");

                    b.Property<double>("Confidence")
                        .HasColumnType("double precision")
                        .HasColumnName("confidence");

                    b.Property<DateTimeOffset>("CreatedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("created_at");

                    b.Property<DateTimeOffset>("ExpiresAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("expires_at");

                    b.Property<string>("ModelId")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("model_id");

                    b.Property<string>("ReasonsJson")
                        .IsRequired()
                        .HasColumnType("jsonb")
                        .HasColumnName("reasons_json");

                    b.Property<int>("Severity")
                        .HasColumnType("integer")
                        .HasColumnName("severity");

                    b.Property<string>("TemplateVersion")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("template_version");

                    b.HasKey("Id");

                    b.HasIndex("CacheKey")
                        .IsUnique();

                    b.HasIndex("ExpiresAt");

                    b.ToTable("local_model_advisor_response_cache", (string)null);
                });

            modelBuilder.Entity("Viegard.Persistence.Postgres.Model.LocalModelAdvisorSettingsRow", b =>
                {
                    b.Property<int>("Id")
                        .HasColumnType("integer")
                        .HasColumnName("id");

                    b.Property<bool>("DeEscalationEnabled")
                        .HasColumnType("boolean")
                        .HasColumnName("de_escalation_enabled");

                    b.Property<double>("DeEscalationMinModelConfidence")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("double precision")
                        .HasDefaultValue(0.69999999999999996)
                        .HasColumnName("de_escalation_min_model_confidence");

                    b.Property<int>("DeEscalationProtectedSeverity")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer")
                        .HasDefaultValue(7)
                        .HasColumnName("de_escalation_protected_severity");

                    b.Property<bool>("Enabled")
                        .HasColumnType("boolean")
                        .HasColumnName("enabled");

                    b.Property<string>("Endpoint")
                        .IsRequired()
                        .HasMaxLength(2048)
                        .HasColumnType("character varying(2048)")
                        .HasColumnName("endpoint");

                    b.Property<bool>("EnsembleEnabled")
                        .HasColumnType("boolean")
                        .HasColumnName("ensemble_enabled");

                    b.Property<int>("InjectionAction")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer")
                        .HasDefaultValue(1)
                        .HasColumnName("injection_action");

                    b.Property<double>("InvokeConfidenceMax")
                        .HasColumnType("double precision")
                        .HasColumnName("invoke_confidence_max");

                    b.Property<double>("InvokeConfidenceMin")
                        .HasColumnType("double precision")
                        .HasColumnName("invoke_confidence_min");

                    b.Property<string>("KeepAlive")
                        .IsRequired()
                        .HasMaxLength(64)
                        .HasColumnType("character varying(64)")
                        .HasColumnName("keep_alive");

                    b.Property<double>("MaxConfidenceDelta")
                        .HasColumnType("double precision")
                        .HasColumnName("max_confidence_delta");

                    b.Property<double>("MaxDownwardConfidenceDelta")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("double precision")
                        .HasDefaultValue(0.10000000000000001)
                        .HasColumnName("max_downward_confidence_delta");

                    b.Property<int>("MaxDownwardSeverityDelta")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer")
                        .HasDefaultValue(1)
                        .HasColumnName("max_downward_severity_delta");

                    b.Property<int>("MaxSeverityDelta")
                        .HasColumnType("integer")
                        .HasColumnName("max_severity_delta");

                    b.Property<string>("Model")
                        .IsRequired()
                        .HasMaxLength(128)
                        .HasColumnType("character varying(128)")
                        .HasColumnName("model");

                    b.Property<bool>("ResponseCacheEnabled")
                        .HasColumnType("boolean")
                        .HasColumnName("response_cache_enabled");

                    b.Property<int>("ResponseCacheTtlHours")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer")
                        .HasDefaultValue(72)
                        .HasColumnName("response_cache_ttl_hours");

                    b.Property<string>("SecondModel")
                        .IsRequired()
                        .HasMaxLength(128)
                        .HasColumnType("character varying(128)")
                        .HasColumnName("second_model");

                    b.Property<string>("SecondModelEndpoint")
                        .IsRequired()
                        .ValueGeneratedOnAdd()
                        .HasMaxLength(2048)
                        .HasColumnType("character varying(2048)")
                        .HasDefaultValue("http://127.0.0.1:11434")
                        .HasColumnName("second_model_endpoint");

                    b.Property<DateTimeOffset?>("SeededAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("seeded_at");

                    b.Property<double>("Temperature")
                        .HasColumnType("double precision")
                        .HasColumnName("temperature");

                    b.Property<int>("TimeoutMs")
                        .HasColumnType("integer")
                        .HasColumnName("timeout_ms");

                    b.Property<DateTimeOffset>("UpdatedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("updated_at");

                    b.Property<string>("UpdatedBy")
                        .IsRequired()
                        .HasMaxLength(128)
                        .HasColumnType("character varying(128)")
                        .HasColumnName("updated_by");

                    b.Property<int>("Version")
                        .HasColumnType("integer")
                        .HasColumnName("version");

                    b.HasKey("Id");

                    b.ToTable("local_model_advisor_settings", null, t =>
                        {
                            t.HasCheckConstraint("CK_local_model_advisor_settings_fixed_id", "id = 1");
                        });
                });

            modelBuilder.Entity("Viegard.Persistence.Postgres.Model.MikroTikRouterRow", b =>
                {
                    b.Property<Guid>("Id")
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<string>("BaseUrl")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("base_url");

                    b.Property<DateTimeOffset>("CreatedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("created_at");

                    b.Property<bool>("Enabled")
                        .HasColumnType("boolean")
                        .HasColumnName("enabled");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("name");

                    b.Property<string>("PasswordCiphertext")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("password_ciphertext");

                    b.Property<string>("PinnedCertificateSha256")
                        .HasColumnType("text")
                        .HasColumnName("pinned_cert_sha256");

                    b.Property<int>("RowVersion")
                        .HasColumnType("integer")
                        .HasColumnName("row_version");

                    b.Property<string>("TransportMode")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("transport_mode");

                    b.Property<DateTimeOffset>("UpdatedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("updated_at");

                    b.Property<string>("UpdatedBy")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("updated_by");

                    b.Property<string>("Username")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("username");

                    b.HasKey("Id");

                    b.HasIndex("Name")
                        .IsUnique();

                    b.ToTable("mikrotik_routers", (string)null);
                });

            modelBuilder.Entity("Viegard.Persistence.Postgres.Model.NormalizedEventRow", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<string>("EntitiesJson")
                        .IsRequired()
                        .HasColumnType("jsonb")
                        .HasColumnName("entities_json");

                    b.Property<DateTimeOffset>("OccurredAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("occurred_at");

                    b.Property<string>("PayloadJson")
                        .IsRequired()
                        .HasColumnType("jsonb")
                        .HasColumnName("payload_json");

                    b.Property<Guid>("RawObservationId")
                        .HasColumnType("uuid")
                        .HasColumnName("raw_observation_id");

                    b.Property<int>("SourceId")
                        .HasColumnType("integer")
                        .HasColumnName("source_id");

                    b.HasKey("Id");

                    b.HasIndex("OccurredAt");

                    b.HasIndex("RawObservationId");

                    b.HasIndex("SourceId");

                    b.ToTable("events", (string)null);
                });

            modelBuilder.Entity("Viegard.Persistence.Postgres.Model.PolicyPostureSettingsRow", b =>
                {
                    b.Property<int>("Id")
                        .HasColumnType("integer")
                        .HasColumnName("id");

                    b.Property<bool>("DryRun")
                        .HasColumnType("boolean")
                        .HasColumnName("dry_run");

                    b.Property<bool>("EmergencyStop")
                        .HasColumnType("boolean")
                        .HasColumnName("emergency_stop");

                    b.Property<bool>("ManualApprovalMode")
                        .HasColumnType("boolean")
                        .HasColumnName("manual_approval_mode");

                    b.Property<int>("RowVersion")
                        .HasColumnType("integer")
                        .HasColumnName("row_version");

                    b.Property<DateTimeOffset>("UpdatedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("updated_at");

                    b.Property<string>("UpdatedBy")
                        .IsRequired()
                        .HasMaxLength(128)
                        .HasColumnType("character varying(128)")
                        .HasColumnName("updated_by");

                    b.HasKey("Id");

                    b.ToTable("policy_posture_settings", null, t =>
                        {
                            t.HasCheckConstraint("CK_policy_posture_settings_fixed_id", "id = 1");
                        });
                });

            modelBuilder.Entity("Viegard.Persistence.Postgres.Model.PolicyRow", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer")
                        .HasColumnName("id");

                    NpgsqlPropertyBuilderExtensions.UseIdentityAlwaysColumn(b.Property<int>("Id"));

                    b.Property<DateTimeOffset>("FirstSeenAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("first_seen_at");

                    b.Property<string>("PolicyKey")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("policy_key");

                    b.Property<string>("PolicyVersion")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("policy_version");

                    b.HasKey("Id");

                    b.HasIndex("PolicyKey", "PolicyVersion")
                        .IsUnique();

                    b.ToTable("policies", (string)null);
                });

            modelBuilder.Entity("Viegard.Persistence.Postgres.Model.PolicyThresholdSettingsRow", b =>
                {
                    b.Property<int>("Id")
                        .HasColumnType("integer")
                        .HasColumnName("id");

                    b.Property<double>("ActionConfidence")
                        .HasColumnType("double precision")
                        .HasColumnName("action_confidence");

                    b.Property<int>("ActionMinSeverity")
                        .HasColumnType("integer")
                        .HasColumnName("action_min_severity");

                    b.Property<double>("ReviewConfidence")
                        .HasColumnType("double precision")
                        .HasColumnName("review_confidence");

                    b.Property<int>("RowVersion")
                        .HasColumnType("integer")
                        .HasColumnName("row_version");

                    b.Property<DateTimeOffset>("UpdatedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("updated_at");

                    b.Property<string>("UpdatedBy")
                        .IsRequired()
                        .HasMaxLength(128)
                        .HasColumnType("character varying(128)")
                        .HasColumnName("updated_by");

                    b.HasKey("Id");

                    b.ToTable("policy_threshold_settings", null, t =>
                        {
                            t.HasCheckConstraint("CK_policy_threshold_settings_fixed_id", "id = 1");
                        });
                });

            modelBuilder.Entity("Viegard.Persistence.Postgres.Model.QueueCounterRow", b =>
                {
                    b.Property<string>("QueueName")
                        .HasColumnType("text")
                        .HasColumnName("queue_name");

                    b.Property<long>("Abandoned")
                        .HasColumnType("bigint")
                        .HasColumnName("abandoned");

                    b.Property<long>("Completed")
                        .HasColumnType("bigint")
                        .HasColumnName("completed");

                    b.Property<long>("Enqueued")
                        .HasColumnType("bigint")
                        .HasColumnName("enqueued");

                    b.HasKey("QueueName");

                    b.ToTable("queue_counters", (string)null);
                });

            modelBuilder.Entity("Viegard.Persistence.Postgres.Model.QueueMessageRow", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint")
                        .HasColumnName("id");

                    NpgsqlPropertyBuilderExtensions.UseIdentityAlwaysColumn(b.Property<long>("Id"));

                    b.Property<bool>("DeadLettered")
                        .HasColumnType("boolean")
                        .HasColumnName("dead_lettered");

                    b.Property<int>("DeliveryCount")
                        .HasColumnType("integer")
                        .HasColumnName("delivery_count");

                    b.Property<DateTimeOffset>("EnqueuedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("enqueued_at");

                    b.Property<DateTimeOffset?>("LeasedUntil")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("leased_until");

                    b.Property<string>("PayloadJson")
                        .IsRequired()
                        .HasColumnType("jsonb")
                        .HasColumnName("payload_json");

                    b.Property<string>("QueueName")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("queue_name");

                    b.HasKey("Id");

                    b.HasIndex("DeadLettered", "EnqueuedAt");

                    b.HasIndex("QueueName", "DeadLettered", "LeasedUntil", "Id");

                    b.ToTable("queue_messages", (string)null);
                });

            modelBuilder.Entity("Viegard.Persistence.Postgres.Model.QueueTelemetryRow", b =>
                {
                    b.Property<string>("InstanceId")
                        .HasColumnType("text")
                        .HasColumnName("instance_id");

                    b.Property<string>("QueueName")
                        .HasColumnType("text")
                        .HasColumnName("queue_name");

                    b.Property<DateTimeOffset>("CapturedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("captured_at");

                    b.Property<int>("DeadLetterCount")
                        .HasColumnType("integer")
                        .HasColumnName("dead_letter_count");

                    b.Property<int>("Depth")
                        .HasColumnType("integer")
                        .HasColumnName("depth");

                    b.Property<int>("InFlight")
                        .HasColumnType("integer")
                        .HasColumnName("in_flight");

                    b.Property<DateTimeOffset?>("OldestPendingEnqueuedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("oldest_pending_enqueued_at");

                    b.Property<long>("TotalAbandoned")
                        .HasColumnType("bigint")
                        .HasColumnName("total_abandoned");

                    b.Property<long>("TotalCompleted")
                        .HasColumnType("bigint")
                        .HasColumnName("total_completed");

                    b.Property<long>("TotalEnqueued")
                        .HasColumnType("bigint")
                        .HasColumnName("total_enqueued");

                    b.HasKey("InstanceId", "QueueName");

                    b.ToTable("queue_telemetry", (string)null);
                });

            modelBuilder.Entity("Viegard.Persistence.Postgres.Model.RawObservationRow", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<string>("IngestOffset")
                        .HasColumnType("text")
                        .HasColumnName("ingest_offset");

                    b.Property<DateTimeOffset>("ObservedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("observed_at");

                    b.Property<string>("PayloadReference")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("payload_reference");

                    b.Property<string>("RawPayload")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("raw_payload");

                    b.Property<int>("SourceId")
                        .HasColumnType("integer")
                        .HasColumnName("source_id");

                    b.HasKey("Id");

                    b.HasIndex("ObservedAt");

                    b.HasIndex("PayloadReference")
                        .IsUnique();

                    b.HasIndex("SourceId");

                    b.ToTable("raw_observations", (string)null);
                });

            modelBuilder.Entity("Viegard.Persistence.Postgres.Model.RetentionSettingsRow", b =>
                {
                    b.Property<int>("Id")
                        .HasColumnType("integer")
                        .HasColumnName("id");

                    b.Property<int?>("ActionsDays")
                        .HasColumnType("integer")
                        .HasColumnName("actions_days");

                    b.Property<int?>("AuditRecordsDays")
                        .HasColumnType("integer")
                        .HasColumnName("audit_records_days");

                    b.Property<int?>("ClassificationsDays")
                        .HasColumnType("integer")
                        .HasColumnName("classifications_days");

                    b.Property<int?>("DeadLetteredQueueMessagesDays")
                        .HasColumnType("integer")
                        .HasColumnName("dead_lettered_queue_messages_days");

                    b.Property<int?>("DecisionsDays")
                        .HasColumnType("integer")
                        .HasColumnName("decisions_days");

                    b.Property<int?>("EventsDays")
                        .HasColumnType("integer")
                        .HasColumnName("events_days");

                    b.Property<int?>("ExpiredAdminSessionsDays")
                        .HasColumnType("integer")
                        .HasColumnName("expired_admin_sessions_days");

                    b.Property<int?>("IncidentsDays")
                        .HasColumnType("integer")
                        .HasColumnName("incidents_days");

                    b.Property<DateTimeOffset?>("LastCycleAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("last_cycle_at");

                    b.Property<string>("LastCycleCountsJson")
                        .HasColumnType("text")
                        .HasColumnName("last_cycle_counts_json");

                    b.Property<int?>("LocalModelAdvisorConsultsDays")
                        .HasColumnType("integer")
                        .HasColumnName("local_model_advisor_consults_days");

                    b.Property<int?>("RawObservationsDays")
                        .HasColumnType("integer")
                        .HasColumnName("raw_observations_days");

                    b.Property<DateTimeOffset?>("SeededAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("seeded_at");

                    b.Property<DateTimeOffset>("UpdatedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("updated_at");

                    b.Property<string>("UpdatedBy")
                        .IsRequired()
                        .HasMaxLength(128)
                        .HasColumnType("character varying(128)")
                        .HasColumnName("updated_by");

                    b.Property<int>("Version")
                        .HasColumnType("integer")
                        .HasColumnName("version");

                    b.HasKey("Id");

                    b.ToTable("retention_settings", null, t =>
                        {
                            t.HasCheckConstraint("CK_retention_settings_fixed_id", "id = 1");
                        });
                });

            modelBuilder.Entity("Viegard.Persistence.Postgres.Model.SessionSecuritySettingsRow", b =>
                {
                    b.Property<int>("Id")
                        .HasColumnType("integer")
                        .HasColumnName("id");

                    b.Property<int>("ResumeStashTtlSeconds")
                        .HasColumnType("integer")
                        .HasColumnName("resume_stash_ttl_seconds");

                    b.Property<int>("RowVersion")
                        .HasColumnType("integer")
                        .HasColumnName("row_version");

                    b.Property<int>("StepUpValiditySeconds")
                        .HasColumnType("integer")
                        .HasColumnName("step_up_validity_seconds");

                    b.Property<DateTimeOffset>("UpdatedAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("updated_at");

                    b.Property<string>("UpdatedBy")
                        .IsRequired()
                        .HasMaxLength(128)
                        .HasColumnType("character varying(128)")
                        .HasColumnName("updated_by");

                    b.HasKey("Id");

                    b.ToTable("session_security_settings", null, t =>
                        {
                            t.HasCheckConstraint("CK_session_security_settings_fixed_id", "id = 1");
                        });
                });

            modelBuilder.Entity("Viegard.Persistence.Postgres.Model.SourceOffsetRow", b =>
                {
                    b.Property<string>("SourceId")
                        .HasColumnType("text")
                        .HasColumnName("source_id");

                    b.Property<string>("Key")
                        .HasColumnType("text")
                        .HasColumnName("key");

                    b.Property<string>("Value")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("value");

                    b.HasKey("SourceId", "Key");

                    b.ToTable("source_offsets", (string)null);
                });

            modelBuilder.Entity("Viegard.Persistence.Postgres.Model.SourceRow", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer")
                        .HasColumnName("id");

                    NpgsqlPropertyBuilderExtensions.UseIdentityAlwaysColumn(b.Property<int>("Id"));

                    b.Property<DateTimeOffset>("FirstSeenAt")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("first_seen_at");

                    b.Property<string>("SourceKey")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("source_key");

                    b.Property<string>("SourceType")
                        .HasColumnType("text")
                        .HasColumnName("source_type");

                    b.HasKey("Id");

                    b.HasIndex("SourceKey")
                        .IsUnique();

                    b.ToTable("sources", (string)null);
                });

            modelBuilder.Entity("Viegard.Persistence.Postgres.Model.ActionRecordRow", b =>
                {
                    b.HasOne("Viegard.Persistence.Postgres.Model.ActionProviderRow", null)
                        .WithMany()
                        .HasForeignKey("ProviderId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();
                });

            modelBuilder.Entity("Viegard.Persistence.Postgres.Model.AdminRecoveryCodeRow", b =>
                {
                    b.HasOne("Viegard.Persistence.Postgres.Model.AdminUserRow", null)
                        .WithMany()
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("Viegard.Persistence.Postgres.Model.AdminSessionRow", b =>
                {
                    b.HasOne("Viegard.Persistence.Postgres.Model.AdminUserRow", null)
                        .WithMany()
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("Viegard.Persistence.Postgres.Model.AdminTotpSecretRow", b =>
                {
                    b.HasOne("Viegard.Persistence.Postgres.Model.AdminUserRow", null)
                        .WithOne()
                        .HasForeignKey("Viegard.Persistence.Postgres.Model.AdminTotpSecretRow", "UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("Viegard.Persistence.Postgres.Model.AdminUserPreferencesRow", b =>
                {
                    b.HasOne("Viegard.Persistence.Postgres.Model.AdminUserRow", null)
                        .WithOne()
                        .HasForeignKey("Viegard.Persistence.Postgres.Model.AdminUserPreferencesRow", "UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("Viegard.Persistence.Postgres.Model.AdminWebAuthnCredentialRow", b =>
                {
                    b.HasOne("Viegard.Persistence.Postgres.Model.AdminUserRow", null)
                        .WithMany()
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("Viegard.Persistence.Postgres.Model.AppPasswordRow", b =>
                {
                    b.HasOne("Viegard.Persistence.Postgres.Model.AdminUserRow", null)
                        .WithMany()
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("Viegard.Persistence.Postgres.Model.AuditRecordRow", b =>
                {
                    b.HasOne("Viegard.Persistence.Postgres.Model.SourceRow", null)
                        .WithMany()
                        .HasForeignKey("SourceId")
                        .OnDelete(DeleteBehavior.Restrict);
                });

            modelBuilder.Entity("Viegard.Persistence.Postgres.Model.ClassificationRow", b =>
                {
                    b.HasOne("Viegard.Persistence.Postgres.Model.ClassifierRow", null)
                        .WithMany()
                        .HasForeignKey("ClassifierId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();
                });

            modelBuilder.Entity("Viegard.Persistence.Postgres.Model.DecisionRow", b =>
                {
                    b.HasOne("Viegard.Persistence.Postgres.Model.PolicyRow", null)
                        .WithMany()
                        .HasForeignKey("PolicyId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();
                });

            modelBuilder.Entity("Viegard.Persistence.Postgres.Model.NormalizedEventRow", b =>
                {
                    b.HasOne("Viegard.Persistence.Postgres.Model.SourceRow", null)
                        .WithMany()
                        .HasForeignKey("SourceId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();
                });

            modelBuilder.Entity("Viegard.Persistence.Postgres.Model.RawObservationRow", b =>
                {
                    b.HasOne("Viegard.Persistence.Postgres.Model.SourceRow", null)
                        .WithMany()
                        .HasForeignKey("SourceId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();
                });
#pragma warning restore 612, 618
        }
    }
}
