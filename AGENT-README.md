...
- Custom detection signatures (D-0029 slice) match with linear Contains/Prefix scans only; never add regex evaluation of untrusted input.  Admin list pages paginate by keyset on the UUIDv7 id (`WHERE id < cursor ORDER BY id DESC`), filter server-side through GET forms, and sort server-side via whitelisted columns with composite `(sort_col, id)` keyset seeks; never OFFSET.
