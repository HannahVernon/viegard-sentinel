...
public sealed class PostgresJetPackDesiredAddressStore(IDbContextFactory<ViegardDbContext> factory) : IJetPackDesiredAddressStore
{
    public async ValueTask<IReadOnlyList<JetPackDesiredAddress>> ListAsync(CancellationToken cancellationToken = default)
    {
        await using var db = await factory.CreateDbContextAsync(cancellationToken).ConfigureAwait(false);
        var rows = await db.JetPackDesiredAddresses
            .AsNoTracking()
    ...
    public async ValueTask<JetPackDesiredAddressRefreshResult> ReplaceSnapshotAsync(
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        await using var db = await factory.CreateDbContextAsync(cancellationToken).ConfigureAwait(false);
        var existingRows = await db.JetPackDesiredAddresses
            .ToListAsync(cancellationToken)
            .ConfigureAwait(false);
        ...
        foreach (var address in normalized)
        {
            ...
            {
                db.JetPackDesiredAddresses.Add(new JetPackDesiredAddressRow
                {
                    Address = address,
        ...
        {
            db.JetPackDesiredAddresses.RemoveRange(removedRows);
        }

        await db.SaveChangesAsync(cancellationToken).ConfigureAwait(false);

        return new JetPackDesiredAddressRefreshResult(
            added,
            updated,
            removedRows.Count,
            normalized.Count);
